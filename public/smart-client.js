(()=>{
  ...
  function installErrorBoundary(){
    const notify=message=>{
      ...
      safeToast(message||'Kivo hit a display problem. Your saved information is still safe.','error');
    };
  ...
  function setBusy(button,busy,label='Working…'){
    if(!button)return;
    if(busy){button.dataset.originalText=button.textContent;button.disabled=true;button.classList.add('is-busy');button.textContent=label}
  ...
  function installCaptureState(){
    ...
    btn.addEventListener('click',()=>{
      if(!navigator.onLine){safeToast('You’re offline. Reconnect before asking Kivo to understand something.','error');return}
      setBusy(btn,true,'Understanding…');
      const watch=setInterval(()=>{if(!btn.disabled){clearInterval(watch);return}const hasToast=$('#toastLayer .toast');if(hasToast||Date.now()-Number(btn.dataset.busySince||Date.now())>12000){setBusy(btn,false);clearInterval(watch)}},400);
      btn.dataset.busySince=String(Date.now());setTimeout(()=>setBusy(btn,false),12000);
    },true);
  }
  ...
  function restoreChat(){
    const chat=$('#chat');if(!chat||chat.dataset.restored==='1')return;chat.dataset.restored='1';
  ...
  function bestOpenItem(){
    try{
      ...
      const score=x=>{
        if(!x.due_date)return 99999;
        const d=new Date(`${x.due_date}T12:00:00`);const days=Math.round((d-today)/86400000);
        return days<0?days-1000:days;
      };
  ...
  function injectFirstRun(){
    ...
    let guide=$('#firstRunGuide');
    let count=1;try{count=(items||[]).length}catch{}
    if(count>0){guide?.remove();return}
    if(guide)return;
  ...
  function applyInboxFilter(){
    ...
    [...list.querySelectorAll('.item-card')].forEach(card=>{
      ...
      const searchOk=!q||text.includes(q);const show=typeOk&&searchOk;card.classList.toggle('filtered-out',!show);if(show)shown++;
    });
  ...
  function installCommandPalette(){
    ...
    const renderCommands=query=>{
      ...
      $('#commandResults')._rows=rows;
    };
    input.addEventListener('input',()=>renderCommands(input.value));
    input.addEventListener('keydown',e=>{
      const buttons=[...$('#commandResults').querySelectorAll('.command-row')];if(!buttons.length)return;
      let i=Math.max(0,buttons.findIndex(b=>b.classList.contains('selected')));
  ...
  function installSettingsExtras(){
    ...
    section.innerHTML=`<div><strong>Quick controls</strong><p>Press <kbd>Ctrl</kbd> + <kbd>K</kbd> anywhere in Kivo to jump, search or add something.</p></div><div class="settings-mini-grid"><span><b>Enter</b><small>Send Ask Kivo</small></span><span><b>Esc</b><small>Close dialogs</small></span><span><b>/</b><small>Open search</small></span></div>`;
    logout.parentNode.insertBefore(section,logout);
  }
  ...
  function installNavState(){
    ...
    $$('.bottom-nav button,.desktop-rail nav button[data-view]').forEach(b=>b.addEventListener('click',()=>requestAnimationFrame(sync)));sync();
  }

  function refreshDynamic(){
    injectFocusCard();injectFirstRun();applyInboxFilter();
  }
  ...
  function init(){
    polishInputs();addSmartBadge();quickPrompts();installAccessibility();installConnectivity();installErrorBoundary();installCaptureState();installChatObserver();installSmarterAsk();restoreChat();installInboxFilters();installCommandPalette();installSettingsExtras();installNavState();observeCards();refreshDynamic();
    addEventListener('pageshow',()=>{polishInputs();refreshDynamic();restoreChat()});
    document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshDynamic()});
  }
