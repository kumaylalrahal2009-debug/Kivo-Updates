...
const corrections=new Map(Object.entries({
  tmr:'tomorrow',tmrw:'tomorrow',tmrrow:'tomorrow',tomorow:'tomorrow',tommorow:'tomorrow',tommorrow:'tomorrow',tdy:'today',todsy:'today',yday:'yesterday',
  ...
  membeship:'membership',memberhsip:'membership',membship:'membership',remid:'remind',remimd:'remind',remeber:'remember',rember:'remember',
  payemnt:'payment',paymnt:'payment',payed:'paid',paied:'paid',reciept:'receipt',recipt:'receipt',deadine:'deadline',dealine:'deadline',dedline:'deadline',
  apointment:'appointment',appoitment:'appointment',appointmnt:'appointment',whats:'what is',wht:'what',wat:'what',wats:'what is',whta:'what',hte:'the',teh:'the',
  jsut:'just',becuase:'because',cos:'because',cuz:'because',rn:'right now',asap:'as soon as possible',pls:'please',plz:'please',needtodo:'need to do',
  upcomming:'upcoming',upcomingg:'upcoming',monht:'month',mth:'month',mnth:'month',yr:'year',wk:'week',cancled:'cancelled',canceled:'cancelled',renewel:'renewal',renwal:'renewal'
}));
function smartCorrect(input){let s=String(input||'').replace(/[\u2018\u2019]/g,"'").replace(/\s+/g,' ').trim();if(!s)return s;s=s.replace(/\b([A-Za-z][A-Za-z']*)\b/g,w=>{const rep=corrections.get(w.toLowerCase());if(!rep)return w;return w[0]===w[0].toUpperCase()?rep.charAt(0).toUpperCase()+rep.slice(1):rep});return s.replace(/\s+([,.!?])/g,'$1').replace(/([!?])\1{2,}/g,'$1$1').trim()}
...
function localDate(iso){if(!iso)return null;const[y,m,d]=String(iso).split('-').map(Number);return y&&m&&d?new Date(y,m-1,d,12):null}
function daysFromToday(iso){const target=localDate(iso);if(!target)return 9999;const a=new Date(),start=new Date(a.getFullYear(),a.getMonth(),a.getDate(),12);return Math.round((target-start)/86400000)}
...
function snapshot(uid){const all=getItems(uid),open=all.filter(x=>x.status==='open'),active=open.slice().sort((a,b)=>daysFromToday(a.due_date)-daysFromToday(b.due_date));const charges=active.filter(x=>x.amount!=null),recurring=charges.filter(x=>x.recurrence),overdue=active.filter(x=>daysFromToday(x.due_date)<0),today=active.filter(x=>daysFromToday(x.due_date)===0),tomorrow=active.filter(x=>daysFromToday(x.due_date)===1),week=active.filter(x=>{const n=daysFromToday(x.due_date);return n>=0&&n<=7}),month=active.filter(x=>{const n=daysFromToday(x.due_date);return n>=0&&n<=30}),done=all.filter(x=>x.status==='done'),cancelled=all.filter(x=>x.status==='cancelled'),avoidable=charges.filter(x=>x.avoidable);return{all,open,active,charges,recurring,overdue,today,tomorrow,week,month,done,cancelled,avoidable}}
...
function lastReferencedItem(uid,s){for(const m of history(uid,8).slice().reverse())for(const x of s.all)if(String(m.content||'').toLowerCase().includes(String(x.title||'').toLowerCase()))return x;return null}
...
function localAssistant(uid,question){
  ...
  if(/(what|anything).*(urgent|priority|important)|what.*first|where.*start|right now/.test(low)){if(s.overdue.length)return`Start with ${itemPhrase(s.overdue[0])}. It’s overdue.${s.today[0]?` After that, ${itemPhrase(s.today[0])}.`:''}`;if(s.today.length)return`I’d handle ${itemPhrase(s.today[0])} first because it’s due today.${s.tomorrow[0]?` Then ${itemPhrase(s.tomorrow[0])}.`:''}`;if(s.tomorrow.length)return`Nothing is urgent today. The next thing worth preparing for is ${itemPhrase(s.tomorrow[0])}.`;return s.active[0]?`Nothing looks urgent. Your next dated item is ${itemPhrase(s.active[0])}.`:`Nothing needs immediate attention right now.`}
  if(/what.*(need|should).*(do|handle)|what.*do.*today|to.?do|my day|plan my|give me.*plan/.test(low)){if(!s.open.length)return`You’re clear — there’s nothing open that needs action right now.`;const p=[...s.overdue,...s.today,...s.tomorrow,...s.week].filter((x,i,a)=>a.findIndex(y=>y.id===x.id)===i).slice(0,3);if(!p.length)return`You have ${s.open.length} open item${s.open.length===1?'':'s'}, but none has a near deadline. I wouldn’t rush anything right now.`;let out=`Your best next move is ${itemPhrase(p[0])}.`;if(p[1])out+=` After that, ${itemPhrase(p[1],{amount:false})}.`;if(p[2])out+=` Then ${itemPhrase(p[2],{amount:false})}.`;return out}
  ...
  if(/(what|how much).*(pay|paying|spend|subscription|charge|bill)|subscriptions?|upcoming charges?/.test(low)){if(!s.charges.length)return`You don’t have any open charges saved right now.`;const total=s.charges.reduce((a,x)=>a+Number(x.amount||0),0),next=s.charges.filter(x=>x.due_date).sort((a,b)=>daysFromToday(a.due_date)-daysFromToday(b.due_date))[0];let out=`You’ve got ${s.charges.length} upcoming charge${s.charges.length===1?'':'s'} totalling ${money(total)}.`;if(s.recurring.length)out+=` Your recurring monthly equivalent is about ${money(monthlyEquivalent(s.recurring))}.`;if(next)out+=` The next one to hit is ${itemPhrase(next)}.`;return out}
...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);
  ...
  if(req.method==='GET'&&url.pathname==='/api/smart/status'){
    const s=session(req);if(!s)return send(res,401,{error:'Please log in.'});const x=snapshot(s.user_id);return send(res,200,{ok:true,mode:OPENAI_API_KEY?'cloud+local':'local',model:OPENAI_API_KEY?AI_MODEL:null,items:x.all.length,open:x.open.length,overdue:x.overdue.length,version:'smart-v2',updateRepo:UPDATE_REPO});
  }
  if(req.method==='GET'&&url.pathname==='/api/admin/smart-health'){
    if(!adminSession(req))return send(res,401,{error:'Admin login required.'});return send(res,200,{ok:true,smartLayer:'v2',aiConnected:!!OPENAI_API_KEY,aiModel:OPENAI_API_KEY?AI_MODEL:null,cloudTimeoutMs:CLOUD_TIMEOUT_MS,database:existsDb(),version:updater.localVersion(),updateRepo:UPDATE_REPO,assistantMessages:countTable('assistant_messages'),analyticsEvents:countTable('analytics_events')});
  }
  if(req.method==='POST'&&url.pathname==='/api/ask'){
    const s=session(req);if(!s)return send(res,401,{error:'Please log in.'});const body=await readJson(req);if(!safeEqual(req.headers['x-csrf-token']||body._csrf,s.csrf))return send(res,403,{error:'Security token expired. Refresh Kivo and try again.'});if(!isPro(s.user_id)&&asksToday(s.user_id)>=15)return send(res,402,{error:'You’ve used today’s 15 free Ask Kivo messages. Kivo Pro removes the daily limit.'});const original=String(body.q||'').trim();if(!original)return send(res,400,{error:'Ask me something first.'});const corrected=smartCorrect(original);remember(s.user_id,'user',corrected);let answer=null,mode='local';try{answer=await cloudAssistant(s.user_id,corrected);if(answer)mode='cloud'}catch(err){console.warn('Kivo AI fallback:',err.name==='AbortError'?'cloud timeout':err.message)}if(!answer)answer=localAssistant(s.user_id,corrected);remember(s.user_id,'assistant',answer);trackAsk(s.user_id,mode);return send(res,200,{answer,corrected_query:corrected,mode});
  }
