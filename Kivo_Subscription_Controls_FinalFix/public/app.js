...
function wire(){
  ...
  $('#settingsBtn').onclick=()=>$('#settingsDialog').showModal();$('#desktopSettings').onclick=()=>$('#settingsDialog').showModal();$('#closeSettings').onclick=()=>$('#settingsDialog').close();$('#logoutBtn').onclick=logout;$('#notifyBtn').onclick=enableNotifications;
...
async function enterApp(){
  ...
  await loadAll();go(currentView||'home');startReminderPolling();
}
...
async function logout(){try{await api('/api/logout',{method:'POST',body:'{}'});csrf='';user=null;items=[];reminders=[];if(reminderPoll)clearInterval(reminderPoll);$('#settingsDialog').close();show('#landing')}catch(err){toast(err.message,'error')}}
...
function startReminderPolling(){if(reminderPoll)clearInterval(reminderPoll);reminderPoll=setInterval(()=>loadReminders(true),60000);loadReminders(true)}
