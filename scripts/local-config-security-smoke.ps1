...
function Pass([string]$Message) { Write-Host "✓ $Message" }

try {
  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'ensure-local-config.ps1') -AppDir $temp
  ...
  if (-not (Test-Path $config)) { Fail 'business-config.bat was not created' }
  if (-not (Test-Path $login)) { Fail 'owner-login.txt was not created' }
  ...
  foreach ($file in @($config, $login)) {
    ...
    if (-not (@($acl.Access | ForEach-Object { $_.IdentityReference.Translate([System.Security.Principal.SecurityIdentifier]).Value }) -contains $currentSid)) {
      Fail "current user is missing from protected ACL for $file"
    }
  }
  ...
  Write-Host "`nKivo local owner configuration security smoke test passed."
}
finally {
