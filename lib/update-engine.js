...
function createUpdateEngine({root,repo,localDesktop=false}){
  ...
  function infoFrom(release,source='unknown'){
    ...
    return{
      ...
      checksumUrl,checksumName:checksum?.name||null,integrity:checksumUrl?'sha256':'missing-checksum',
      repo,source,reason:assetUrl?(!checksumUrl?'checksum_asset_missing':null):'update_asset_missing'
    };
  }
  ...
  async function verifyChecksum(file,checksumUrl){
    if(!checksumUrl)throw new Error('Kivo release is missing the required SHA-256 checksum. The update was not installed.');
  ...
  async function download(url,dest,checksumUrl){
    if(!checksumUrl)throw new Error('Kivo release is missing the required SHA-256 checksum. The update was not downloaded.');
  ...
  async function install(serverPid){
    ...
    if(!info.checksumUrl)throw new Error('The latest Kivo release is missing its required SHA-256 checksum. Installation was blocked.');
