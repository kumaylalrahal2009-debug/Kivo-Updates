...
function createAccountService({dataDir,uploadsDir,secureCookies=false}){
  const dbPath=path.join(dataDir,'kivo.db');
  const db=new DatabaseSync(dbPath);
  try{db.exec('PRAGMA foreign_keys=ON; PRAGMA busy_timeout=5000;')}catch{}
  ...
  function deleteAccount(req,body,s){
    if(!csrfValid(req,body,s))return response(403,{error:'Security token expired. Refresh Kivo and try again.'});
  ...
  async function handle(req,url,bodyBuffer=null){
    ...
    try{
      ...
      if(method==='DELETE'&&pathname==='/api/account')return deleteAccount(req,body,s);
  ...
  return{handle,close:()=>{try{db.close()}catch{}}};
}
